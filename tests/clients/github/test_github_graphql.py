...
def test_get_corporate_emails_for_user_with_pagination(requests_mock, fixture):
    requests_mock.post("https://api.github.com/graphql",
        json=MOCK_RESPONSE_WITH_PAGINATION)
    requests_mock.post(
        "https://api.github.com/graphql",
        json=MOCK_RESPONSE,
        additional_matcher=lambda req: json.loads(req.text)["variables"]["after"] == "Y3Vyc29yOjEwMA==")

    emails = fixture.get_corporate_emails_for_user("drcox")

    assert emails == [ "perry.cox@sacredheart.com", "perry.cox+ratemydoc@sacredheart.com"]
